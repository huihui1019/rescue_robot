 ...
 */
void PART1(){
	if( robot_->team_color == RobotColor::BLUE){a = 3;b = 0; c = 1;}// 蓝方（通过读取拨码快速切换） 
	else{a = 0;b = 3; c = 2;}// 红方（通过读取拨码快速切换）
	while(robot_->ctrl_state == AUTO_OPERATION){
		while (robot_->ctrl_state == AUTO_OPERATION){
				// if (notfind !=0){
...
void PART2(){
	if(io::getIN1()){a = 3;b = 0; c = 1;}// 红方（通过读取拨码快速切换） 
	else{a = 0;b = 3; c = 2;}// 蓝方（通过读取拨码快速切换）
	while(robot_->ctrl_state == AUTO_OPERATION){
		while (robot_->ctrl_state == AUTO_OPERATION){
				if (notfind !=0 ){
				switch (notfind)//通过switch的值判断到那个找球点位
