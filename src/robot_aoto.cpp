...
#define _SET_SPEED_TIMEOUT 10
	...
	void Turn_angle()
	{
		...
		{ // 自动模式下
			if (fabs(robot.target_pos.yaw - robot.current_pos.yaw) < 1)
			{		   // 判断目标角度和现在角度差值绝对值是否小于1度
				break; // 退出循环
			}
			delay(10); // 延时10ms
		}
	...
	int certain_ball(Robot_t *robot, uint8_t color, uint8_t pd)
	{
		...
		while (robot_->ctrl_state == AUTO_OPERATION) // 自动模式下
		{
			...
			for (uint8_t i = 0; i < 14; i++) // for循环遍历14次
			{
				...
				if (pd == 1)
				{																			   // 判断是color颜色还是除color颜色以外（1不是color颜色）
					if (color != ball_tmp->type && ball_tmp->type != 1 && ball_tmp->type != 2) // 判断视觉数据和color是否不同，且不等于1和2
					{
					...
					if (color == ball_tmp->type && ball_tmp->type != 1 && ball_tmp->type != 2) // 判断视觉数据和color是否相同，且不等于1和2
					{
						dt = (millis() - ball_tmp->timestamp); // 计算现在时间与数据获取时间的插值
						if (dt < 100)						   // 判断dt是否小于100ms
						{
	...
	bool certain_home(Robot_t *robot, uint8_t color, uint8_t pd)
	{
		...
		while (robot_->ctrl_state == AUTO_OPERATION) // 自动模式下
		{
			...
			for (uint8_t i = 0; i < 14; i++) // for循环遍历14次
			{
				...
				ball_tmp = visual::getBall(i); // 获取视觉下发数据
				if (pd == 0)
				{								 // 判断是color颜色还是除color颜色以外（0是color颜色）
	...
	{
		bool pd1;
		if (fabs((robot_->current_pos.yaw) + 90) < 75)
		{												   // 判断目前角度是否满足90度摆放
		...
		else if (fabs((robot_->current_pos.yaw)) < 60)
		{												  // 判断车的姿态绝对值是为-60-60度
			robot.imu_hold = true;						  // 陀螺仪保持
			robot.chassis_mode = POSTION_MODE;			  // 小车进入里程计模式
			robot_->target_pos.x = robot_->current_pos.x; // x目标位置为现在位置
		...
		else if (fabs(fabs(robot_->current_pos.yaw) - 180) < 60)
		{												   // 判断车的姿态绝对值是否为120-240度
			robot.imu_hold = true;						   // 陀螺仪保持
			robot.chassis_mode = POSTION_MODE;			   // 小车进入里程计模式
			robot_->target_pos.x = robot_->current_pos.x;  // x目标位置为现在位置
			robot_->target_pos.y = robot_->current_pos.y;  // y目标位置为现在位置
	...
	bool VisualFindBall(Robot_t *robot, uint8_t color, uint8_t pd)
	{
		...
		while (1)
		{
			...
			for (uint8_t i = 0; i < 14; i++) // for循环遍历14次
			{
				...
				{ // 判断是color颜色还是除color颜色以外（1为除color颜色以外）
					if (ball_tmp->type != 1 && ball_tmp->type != 2)
					{															 // 先把安全区数据过滤
						if (color != ball_tmp->type && color != 1 && color != 2) // 判断视觉数据是否与color不同
						{
							dt = (millis() - ball_tmp->timestamp); // 计算现在时间与数据获取时间的插值
							if (dt < 100)						   // 判断是否小与100ms（最新消息）
							{
		...
		while (robot_->ctrl_state == AUTO_OPERATION)
		{
			while (robot_->ctrl_state == AUTO_OPERATION)
			{
				...
				if (pd)
				{
					...
					// delay(20);// 延时20ms
					// SetSpeed(0.6, 0, 0, 0.45); // 向前冲一段
					SetSpeed(0.7, 0, 0, 0.4); // 向前冲一段
					// delay(20);// 延时20ms
					robot.servo[0]->setAngle(-16); // 爪子合拢（框放下）的角度
					robot.servo[1]->setAngle(13);  // 爪子合拢（框放下）的角度
					// delay(300);// 延时300ms
			...
			while (robot_->ctrl_state == AUTO_OPERATION)
			{
				...
				if (pd)
				{
					...
					corner_angle();					// 调整车身姿态
					delay(300);						// 延时300ms
					robot.servo[0]->setAngle(30);	// 爪子合拢（框放下）的角度
					robot.servo[1]->setAngle(-30);	// 爪子合拢（框放下）的角度
					delay(500);						// 延时500ms
