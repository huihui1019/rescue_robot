...
void updata(void)
{
    ...
    {
        rt_pwm_set(SG4_pwm_dev, SG4_PWM_DEV_CHANNEL, TIME2_PWM_PERIOD, SG4_PWM_START_VALUE);
...
int getIN1()
{
    return rt_pin_read(IN1_PIN);
}

int getIN2()
{
    return rt_pin_read(IN2_PIN);
}

int getIN3()
{
    return rt_pin_read(IN3_PIN);
}

int getIN4()
{
    return rt_pin_read(IN4_PIN);
}
...
void init(void)
{
    ...
    size_t save_len;
    if (ef_get_env_blob("gyro_offset", gyro_offset, sizeof(gyro_offset), &save_len) == 0 || save_len <= 1)
    {
    ...
    temp_pid.Init(&temp_now, &temp_output, &temp_max,
        TEMP_PID_KP, TEMP_PID_KI, TEMP_PID_KD, _PID_CD_DIRECT);
...
void update(void)
{
    ...
    if (dt > 1000/IMU_UPDATE_FREQUENCY)
    {
        ...
        /* AHRS */
        ahrs.updateIMU(gx - gyro_offset[0], gy - gyro_offset[1], gz - gyro_offset[2], ax, ay, az, (float)dt/1000.0);
        ...
        temp_pid.Compute();
        rt_pwm_set(temp_pwm_dev, TEMP_PWM_DEV_CHANNEL, TIME2_PWM_PERIOD, temp_output);
    }
}
...
struct sensor_3_axis *getGyro()
{
    return &gyro_data.data.gyro;
}

void reset(void)
{
    ahrs.setQuaternion(1.0f, 0.0f, 0.0f, 0.0f);
}
...
//     printf("balls:%d %d %d %d %d\n", balls[msg->frame_id].type,balls[msg->frame_id].x, balls[msg->frame_id].y, 
//                                     balls[msg->frame_id].w, balls[msg->frame_id].h);
	//printf("ball:%d\n",(160-(balls[msg->frame_id].x + balls[msg->frame_id].w/2))*(160-(balls[msg->frame_id].x + balls[msg->frame_id].w/2))+(160-(balls[msg->frame_id].y + balls[msg->frame_id].h/2))*(160-(balls[msg->frame_id].y + balls[msg->frame_id].h/2)));
