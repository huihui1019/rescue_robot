...
void AutoSwitch(Robot_t *robot, Gamepad_t *gamepad)
{
    ...
    /* 手柄遥感按下，启动自动代码 */
    if (IS_BUTTON_CLICKED(gamepad->buttons, robot->last_buttons, GAMEPAD_XBOX_BUTTON_THUMBL)
    || IS_BUTTON_CLICKED(gamepad->buttons, robot->last_buttons, GAMEPAD_XBOX_BUTTON_THUMBR))
    {
