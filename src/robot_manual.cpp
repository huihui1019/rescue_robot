...
void General(Robot_t *robot, Gamepad_t *gamepad) {
  if (IS_BUTTON_CLICKED(gamepad->buttons, robot->last_buttons,
  ...
  {
    io::setSG4(1,
               gamepad->rt); //*涵道手动大小  io::setSG4(1,int((gamepad->rt)/2))
                             //风力减小两倍*//
  }
#endif
}
...
void AutoSwitch(Robot_t *robot, Gamepad_t *gamepad) {
  ...
  /* 手柄遥感按下，启动自动代码 */
  if (IS_BUTTON_CLICKED(gamepad->buttons, robot->last_buttons,
                        GAMEPAD_XBOX_BUTTON_THUMBL) ||
      IS_BUTTON_CLICKED(gamepad->buttons, robot->last_buttons,
                        GAMEPAD_XBOX_BUTTON_THUMBR)) {
...
void ChassisSpeed(Robot_t *robot, Gamepad_t *gamepad) {
  ...
  robot->set_vel.linear_x =
      fmap(gamepad->y, INT16_MIN, INT16_MAX, -CHASSIS_MAX_LINEAR_SPEED,
           CHASSIS_MAX_LINEAR_SPEED); // m/s
  robot->set_vel.linear_y =
      -fmap(gamepad->x, INT16_MIN, INT16_MAX, -CHASSIS_MAX_LINEAR_SPEED,
            CHASSIS_MAX_LINEAR_SPEED); // m/s
  robot->set_vel.angular_z =
      -fmap(gamepad->z, INT16_MIN, INT16_MAX, -CHASSIS_MAX_ANGULAR_SPEED,
            CHASSIS_MAX_ANGULAR_SPEED); // rad/s
  ...
  if (gamepad->hat & GAMEPAD_XBOX_HAT_UP) {
    ...
    (ROBOT_CHASSIS == ROBOT_CHASSIS_THREE_WHEEL_OMNI)
  if (gamepad->hat & GAMEPAD_XBOX_HAT_LEFT) {
    ...
    (ROBOT_CHASSIS == ROBOT_CHASSIS_FOUR_DIFF)
  if (gamepad->hat & GAMEPAD_XBOX_HAT_LEFT) {
  ...
  if (robot->imu_hold) {
    ...
    constrain(tmp, -degrees(CHASSIS_MAX_ANGULAR_SPEED),
              degrees(CHASSIS_MAX_ANGULAR_SPEED));
