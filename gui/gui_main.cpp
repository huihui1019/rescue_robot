...
const char *battery_anim[5] = {LV_SYMBOL_BATTERY_EMPTY, LV_SYMBOL_BATTERY_1, LV_SYMBOL_BATTERY_2, LV_SYMBOL_BATTERY_3, LV_SYMBOL_BATTERY_FULL};
...
static void timer_handler(lv_timer_t * timer)
{
    ...
    if (millis() - tick[0] > 100)
    {
        ...
        lv_label_set_text_fmt(ui_yaw_label, "Yaw: %.2f(deg)", robot.current_pos.yaw);
    }
    ...
    if (millis() - tick[1] > 1000)
    {
        tick[1] = millis();
        int battery_index;
        for (battery_index = 4; battery_index > 0 &&robot.battery_voltage < voltage_thresholds[battery_index]; --battery_index) {}
        if (robot.battery_voltage == 0) {
            battery_index = 0;
        }
        lv_label_set_text_fmt(ui_battery_label, "%.1fv %s", robot.battery_voltage, battery_anim[battery_index]);
    }
}
...
void gui_main_init(lv_obj_t* root)
{
    ...
    lv_obj_set_style_bg_color(ui_robot_state_panel, lv_color_hex(0xC0FF6B), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_opa(ui_robot_state_panel, 255, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_border_color(ui_robot_state_panel, lv_color_hex(0xD5D5D5), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_border_opa(ui_robot_state_panel, 255, LV_PART_MAIN | LV_STATE_DEFAULT);

    ui_robot_state_label = lv_label_create(ui_robot_state_panel);
    lv_obj_set_align(ui_robot_state_label, LV_ALIGN_CENTER);
    lv_label_set_text(ui_robot_state_label, "S");
    lv_obj_set_style_text_font(ui_robot_state_label, &lv_font_montserrat_48, LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    lv_obj_set_style_bg_color(ui_num_panel, lv_color_hex(0x638FF4), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_opa(ui_num_panel, 255, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_border_color(ui_num_panel, lv_color_hex(0xD5D5D5), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_border_opa(ui_num_panel, 255, LV_PART_MAIN | LV_STATE_DEFAULT);

    ui_num_label = lv_label_create(ui_num_panel);
    lv_obj_set_align(ui_num_label, LV_ALIGN_CENTER);
    lv_label_set_text(ui_num_label, "0");
    lv_obj_set_style_text_font(ui_num_label, &lv_font_montserrat_48, LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    lv_obj_set_style_text_font(ui_textarea, &ui_font_dingtalk, LV_PART_MAIN | LV_STATE_DEFAULT);
